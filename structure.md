...
[사용자 브라우저]
       │
       ├── 정적 파일 (HTML/CSS/JS/JSON) ──→ [NAS WebStation :443] → front/
       │
       └── API 요청 (/api/*) ──→ [Nginx :8080] ──→ [Backend :3000] ──→ [MySQL :3306]
                                      │
...
|-----------|--------|------|-------------|-----------|
| `wararchive-mysql` | `mysql:8.0` | 3306 (내부) | 512MB | `no-new-privileges` |
| `wararchive-backend` | 빌드 이미지 | 3000 (내부) | 256MB | `read_only`, `no-new-privileges` |
| `wararchive-nginx` | `nginx:alpine` | 8080→80 | 128MB | `read_only`, `no-new-privileges` |
